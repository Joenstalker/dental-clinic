...
<body class="hold-transition sidebar-mini">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="selectedDoctor" required>
              <option value="{{ $appointment->user->id }}" selected>{{ $appointment->user->full_name }} (Current Session Doctor)</option>
              @foreach($dentists as $dentist)
                @if($dentist->id != $appointment->user->id)
                  <option value="{{ $dentist->id }}">{{ $dentist->full_name }}</option>
                @endif
              @endforeach
            </select>
            <small class="form-text text-muted">Note: This appointment is for the session created by {{ $appointment->user->full_name }}. To book with a different doctor, please select their session from the calendar.</small>
          </div>
  ...
  <script>
    ...
    const selectedDoctorName = document.getElementById('selectedDoctor').options[document.getElementById('selectedDoctor').selectedIndex].text;
    const appointmentSessionId = document.querySelector('input[value="{{ $appointment->id }}"]').value;

    Swal.fire({
        title: 'Confirm Appointment',
        html: `Are you sure you want to book an appointment with <strong>${selectedDoctorName}</strong> for ${selectedDate} at ${selectedSlot}?`,
        icon: 'question',
