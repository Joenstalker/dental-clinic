...
<body class="hold-transition sidebar-mini">
        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title" style="font-size: 2em">Assistant's List</h3>
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        <form action="{{ route('admin.assistant-delete', $assistant->id) }}" method="post" style="display:inline;">
                          @csrf
                          <button type="submit" class="btn btn-danger" onclick="event.preventDefault(); deleteUser(event, this);">Delete</button>
                        </form>
                      </td>
                    </tr>
                  @endforeach
  ...
  <script>
        ...
        $('#addDentistForm').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('admin.add-assistant') }}", // Laravel route to handle the request
                method: 'POST',
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
                    if (response.success === true) {
                        // If assistant is added successfully, show a success message and close the modal
                        $('#addAccountModal').modal('hide');
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: 'Assistant account has been registered successfully!',
