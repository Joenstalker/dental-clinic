...
class DentistController extends Controller
{
    ...
    public function session()
    {
        ...
        $services = \App\Models\Services::orderBy('service', 'asc')->get();
        
        // Return the view with the necessary data
        return view('dentist.session', compact('currentDate', 'sessions', 'services'));
    }
    ...
    public function addSession(Request $request)
    {
        ...
        $request->validate([
            ...
            'number_of_member' => 'required|integer|min:1|max:19',
            'price' => 'required|numeric|min:0',
        ]);
    
        $dentist = auth()->user();
        
        // Use custom session title if provided, otherwise use selected service
        $sessionTitle = $request->input('custom_session_title') 
            ? $request->input('custom_session_title') 
            : $request->input('session_title');
