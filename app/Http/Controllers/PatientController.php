...
class PatientController extends Controller
{
    ...
    public function bookAppointment(Request $request)
    {
        ...
        $appointmentSession = AppointmentSession::findOrFail($request->appointment_session);
        
        // Check authorization to view this session
        $this->authorize('view', $appointmentSession);
    ...
    public function appointment()
    {
        ...
        // Fetch all active dentists for direct booking dropdown
        $dentists = User::where('userRole', 'dentist')
                        ->where('status', 'active')
                        ->get(['id', 'full_name']);
    
        return view('patient.appointment', compact('currentDate', 'dentists'));
    }
    ...
    public function appointmentDetails($id)
    {
        ...
        // Fetch all active dentists for dropdown
        $dentists = User::where('userRole', 'dentist')
                        ->where('status', 'active')
                        ->get(['id', 'full_name']);

        // Otherwise, return the appointment details view
        return view('patient.appointment-details', compact('appointment', 'currentDate', 'takenSlots', 'dentists'));
    }
    ...
    public function directBooking(Request $request)
    {
        ...
        $user = Auth::user();
        $dentist = User::findOrFail($request->dentist);

        // Check if dentist is active
        if ($dentist->status !== 'active' || $dentist->userRole !== 'dentist') {
            return response()->json([
                'success' => false,
        ...
        // Check if the selected date and time is already booked for this dentist
        $selectedDateTime = Carbon::parse($request->appointment_date . ' ' . explode(' - ', $request->appointment_time)[0]);
        
        // Check for existing bookings at the same time with the same dentist
        $existingBooking = Member::whereHas('appointmentSession', function($query) use ($dentist, $request) {
            $query->where('user_id', $dentist->id)
                  ->whereDate('schedule_date', $request->appointment_date);
        })
        ->where('time', $request->appointment_time)
        ...
        $userExistingBooking = Member::where('user_id', $user->id)
            ->whereHas('appointmentSession', function($query) use ($request) {
                $query->whereDate('schedule_date', $request->appointment_date);
            })
            ->where('time', $request->appointment_time)
            ->where('status', '!=', 'disapproved')
            ->first();
        ...
        // Create appointment session on-the-fly (or find existing one for that date)
        $appointmentDate = Carbon::parse($request->appointment_date)->format('Y-m-d H:i:s');
        
        // Check if there's already a session for this dentist on this date
        $existingSession = AppointmentSession::where('user_id', $dentist->id)
            ->whereDate('schedule_date', $request->appointment_date)
            ->first();

        if ($existingSession) {
            // Use existing session if available slots
            if ($existingSession->number_of_member <= 0) {
                return response()->json([
                    'success' => false,
                    'message' => 'No available slots for this date. Please select another date.',
                ], 400);
            }
            $appointmentSession = $existingSession;
        } else {
            // Create new session with default values
            $appointmentSession = AppointmentSession::create([
                'user_id' => $dentist->id,
                'session_title' => $request->service,
                'schedule_date' => $appointmentDate,
                'number_of_member' => 10, // Default slots
                'price' => 0, // Can be set later
            ]);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Appointment booked successfully! Your appointment is pending approval.',
            'data' => $member,
        ]);
    }
}
